...
| Base Postgres | VM `lifeai-db` en `us-central1-a` | VM `lifeai-db` en `us-west1-b` (voir §6 — pénurie de stock sur `us-central1-*`) |
| Client OAuth Google | Un seul, réutilisé au départ, puis **remplacé par un client dédié** (voir §7) | Client OAuth propre au projet `lifeai-devops` |
...
| `secrets.tf` | Les 6 secrets Secret Manager (voir §4) |
...
## 4. Secrets — trois niveaux différents
...
| `lifeai-twilio-*`, `lifeai-oauth-client-json` | Secret Manager | Valeur fournie en entrée (Twilio existant, client OAuth dédié — voir §7), stockée par Terraform |
...
| `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `OAUTH_CLIENT_JSON` | **GitHub** Actions secrets | Fournis manuellement, transmis à Terraform via `TF_VAR_*` (voir incident §6) |
...
## 5. `bootstrap/bootstrap_ci.sh` — le seul script bash restant
...
## 6. Incidents rencontrés pendant la mise en place (et corrections)
...
## 7. Ce qui reste manuel (volontairement — Google ne l'expose pas en API)
...
| Tester un `plan` sans déployer | `cd terraform && terraform init && terraform plan` (nécessite les 3 variables sensibles, via `terraform.tfvars` local gitignored ou `TF_VAR_*`) |
